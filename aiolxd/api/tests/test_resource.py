...
class TestResource:
    ...
    def test_eq(self):
        ...
        remote = FakeRemote()
        assert (
            SampleResource(remote, '/resource') == SampleResource(
                remote, '/resource'))

    def test_eq_false(self):
        """Resources are not equal with different remotes or URI."""
        assert (
            SampleResource(FakeRemote(), '/resource1') != SampleResource(
                FakeRemote(), '/resource1'))
        remote = FakeRemote()
        assert (
            SampleResource(remote, '/resource1') != SampleResource(
                remote, '/resource2'))
    ...
    @pytest.mark.parametrize(
        'path,resource_id', [
            ('/resource/myresource', 'myresource'), ('/', None),
    ...
    async def test_delete(self):
        ...
        assert remote.calls == [
            (('DELETE', '/resource', None, None, None, None))
        ]
